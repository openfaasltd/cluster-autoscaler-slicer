...
func (g *SlicerNodeGroup) IncreaseSize(delta int) error {
	...
	// 	# 1) Create persistent sysctl drop-in
	// cat <<EOF >/etc/sysctl.d/99-custom.conf
	// # Added by userdata on $(date)
	...
	// # 2) Create persistent limits drop-in
	...
	// --with-node-id is required for automated node addition/removal
	// in order to prevent stale nodes in the K3s server cache from rejecting
	// new nodes with a previously used hostname i.e. nodegroup-1 is added, removed and then added again
